 ...
 */
public class CustomDefaultClusterRenderer<T extends ClusterItem> implements ClusterRenderer<T>
{
	private static final String TAG = CustomDefaultClusterRenderer.class.getSimpleName();
	...
	public CustomDefaultClusterRenderer(Context context, GoogleMap map, ClusterManager<T> clusterManager) {
		mMap = map;
		mDensity = context.getResources().getDisplayMetrics().density;
		mIconGenerator = new IconGenerator(context);
		//LayoutInflater myInflater = (LayoutInflater)context.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
		//View activityView = myInflater.inflate(R.layout.ginkome_custom_marker, null, false);
	...
	private LayerDrawable makeClusterBackground(ShapeDrawable mColoredCircleBackground) {
		ShapeDrawable outline = new ShapeDrawable(new OvalShape());
		outline.getPaint().setColor(0x80ffffff); // Transparent white.
		LayerDrawable background = new LayerDrawable(new Drawable[]{outline, mColoredCircleBackground});
		int strokeWidth = (int) (mDensity * 3);
	...
	protected int getType(Cluster<T> cluster) {
		Log.d(TAG, "Cluster type: " + (cluster instanceof SproutSearchItem ? ((SproutSearchItem) cluster).getType() : "not typed"));
		int type = -1;
		for (T c:cluster.getItems()){
			if (c instanceof SproutSearchItem)
			{
				type = ((SproutSearchItem) c).getType();
				break;
			}
		}
	...
	private class RenderTask implements Runnable {
		...
		@SuppressLint("NewApi")
		public void run() {
			/*if (clusters.equals(CustomDefaultClusterRenderer.this.mClusters)) {
				mCallback.run();
				return;
			}*/
			...
			if (CustomDefaultClusterRenderer.this.mClusters != null && SHOULD_ANIMATE) {
				existingClustersOnScreen = new ArrayList<Point>();
				for (Cluster<T> c : CustomDefaultClusterRenderer.this.mClusters) {
			...
			mMarkers = newMarkers;
			CustomDefaultClusterRenderer.this.mClusters = clusters;
			mZoom = zoom;
	...
	protected void onBeforeClusterRendered(Cluster<T> cluster, MarkerOptions markerOptions) {
		int type = getType(cluster);
		int bucket = getBucket(cluster);
		BitmapDescriptor descriptor = null;//mIcons.get(type);
		if (descriptor == null) {
			if (type == 3)
				mColoredCircleBackground.getPaint().setColor(Color.GREEN);
			else
				mColoredCircleBackground.getPaint().setColor(Color.rgb(163, 73, 164));
