 ...
 */
public class CustomAlgo<T extends ClusterItem> implements Algorithm<T>
{
	private static final String TAG=CustomAlgo.class.getSimpleName();
	 ...
	 * Any modifications should be synchronized on mQuadTree.
	 */
	private final CustomPointQuadTree<QuadItem<T>> mQuadTree = new CustomPointQuadTree<QuadItem<T>>(0, 1, 0, 1);
	...
	public Set<? extends Cluster<T>> getClusters(double zoom) {
		...
		final Map<QuadItem<T>, CustomStaticCluster<T>> itemToCluster = new HashMap<QuadItem<T>, CustomStaticCluster<T>>();
		...
		synchronized (mQuadTree) {
			for (QuadItem<T> candidate : mItems) {
				...
				clusterItems = mQuadTree.search(candidate.getType(),searchBounds);
				...
				CustomStaticCluster<T> cluster = new CustomStaticCluster<>(candidate.getType(),candidate.mClusterItem.getPosition());
				results.add(cluster);

				List<QuadItem<T>> visited = new ArrayList<>();
				for (QuadItem<T> clusterItem : clusterItems) {
					visited.add(clusterItem);
				...
				visitedCandidates.addAll(visited);
			}
		}
		return results;
	}
	...
	private static class QuadItem<T extends ClusterItem> implements CustomPointQuadTree.Item, Cluster<T>,TypedClusterItem {
		...
		public int getType()
		{
			return mClusterItem!=null&&mClusterItem instanceof TypedClusterItem?((TypedClusterItem) mClusterItem).getType():-1;
		}
	}
}