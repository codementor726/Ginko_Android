...
public class CustomPointQuadTree<T extends CustomPointQuadTree.Item> {
     ...
     */
    private void split() {
        mChildren = new ArrayList<CustomPointQuadTree<T>>(4);
     ...
     */
    public Collection<T> search(int type,Bounds searchBounds) {
        final List<T> results = new ArrayList<T>();
        search(type,searchBounds, results);
        return results;
    }

    private void search(int type,Bounds searchBounds, Collection<T> results) {
        ...
        if (this.mChildren != null) {
            for (CustomPointQuadTree<T> quad : mChildren) {
                quad.search(type,searchBounds, results);
            }
        } else if (mItems != null) {
            /*if (searchBounds.contains(mBounds)) {
                ...
                for (T item : mItems) {
                    int existtype = item instanceof TypedClusterItem?((TypedClusterItem) item).getType():-1;
                    if (existtype!=type)
                        continue;
