...
public  class CountryCodeIndexCursor implements Cursor {

    private ListAdapter adapter;
    private int position;
//    private Map<String, String> map;

    public CountryCodeIndexCursor(ListAdapter adapter){
    ...
    public String getString(int columnIndex) {
        CountryCodeItem item = (CountryCodeItem)adapter.getItem(position);
        if (item.isSection()){
            return item.getSectionName();
        }
        if (item.strCountryName.length() < 1) {
            return "";
        }
        return item.strCountryName.substring(0,1);
    }
