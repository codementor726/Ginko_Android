...
public class GPUImage {
	...
	private final VideoRender mVideoRender;
	...
	public GPUImage(final Context context) {
		...
		mVideoRender = new VideoRender(mContext, mFilterPos);
    }
	...
	public void setVideoGLSurfaceView(final GLSurfaceView view) {
		mGlSurfaceView = view;
		mGlSurfaceView.setEGLContextClientVersion(2);
	...
	public void setUpVideoPlayer(final MediaPlayer mp, final int degrees, final boolean flipHorizontal, final boolean flipVertical) {
		mGlSurfaceView.setRenderMode(GLSurfaceView.RENDERMODE_CONTINUOUSLY);
		//mRenderer.setUpSurfaceTexture(mp);
		//mVideoRender.setMediaPlayer(mp);
    ...
    public void setFilterPosition(int position){
        mVideoRender.setShader(position);
        requestRender();
    }
    /**
    ...
    public void saveToPictures(final Bitmap bitmap, final String folderName, final String fileName,
        ...
        {
            new SaveTask(bitmap, folderName, fileName, listener).executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR);
        }
        else
    ...
    /*
     * When record the preview frames filter applied , set record flag  
     */
    
    public synchronized void setReocrding(boolean isRecording)
    {
    	if(mRenderer != null)
    	{
    		mRenderer.setRecording(isRecording);
    	}
    }
    
    /*
     * called when one frame is recorded
     */
    public void setOnFrameRecordListener(FrameRecordListener recordListener)
    {
    	if(mRenderer != null)
    	{
    		mRenderer.setOnFrameRecordListener(recordListener);
    	}
    }
    @Deprecated
    private class SaveTask extends AsyncTask<Void, Void, Void> {
        ...
        private void saveImage(final String folderName, final String fileName, final Bitmap image) {
            File file = null;
            if(fileName.contains("/"))//if fileName is absoulte full file path , then create file from this file path
            {
    ...
    public void releaseGPUImageFrameBuffers()
    {
    	mRenderer.releaseGLFrameBuffer();
    }
	...
	public void startVideoRecording(String filePath) {
        //Log.i("RecordVideo", "StartVideoRecording: " + filePath);
		if (mRecorder == null) {
            //Log.i("RecordVideo", "Recorder Format Success!");
			//mRecorder = new MyRecorder(filePath, mRenderer.getFrameWidth(), mRenderer.getFrameHeight());
			mRecorder = new MyRecorder(filePath, mVideoWidth, mVideoHeight);
			mRecorder.prepareEncoder();
			//mRenderer.setRecorder(mRecorder, mRenderer.getFrameWidth(), mRenderer.getFrameHeight());
			mRenderer.setRecorder(mRecorder, mVideoWidth, mVideoHeight);
		}
	}

	public void stopVideoRecording() {
        //Log.i("RecordVideo", "StopVideoRecording");
		if (mRecorder != null) {
            //Log.i("RecordVideo", "Recorder Stop Success");
			mRecorder.stop();
			mRecorder = null;
			//mRenderer.setRecorder(null, mRenderer.getFrameWidth(), mRenderer.getFrameHeight());
			mRenderer.setRecorder(null, mVideoWidth, mVideoHeight);
		}
	}

	public void setVideoSize(int width, int height) {
		mVideoWidth = width;//mRenderer.getFrameWidth();
		mVideoHeight = height;//mRenderer.getFrameHeight();
	}

    public void setVideoSized(int width, int height) {
        mVideoWidth = width;
        mVideoHeight = height;
    }
	...
	public void onDestroyVideoRender() {
	...
	public void onPlay() {
		//mRenderer.onPlay();
        mVideoRender.onPlay(false);
	}
    public void onPlayNoneMute() {
        //mRenderer.onPlay();
        mVideoRender.onPlay(true);
    }
    public void onStop(int reset)
    {
        mVideoRender.onStop(reset);
    }
    public void reset()
    {
        mVideoRender.reset();
    ...
    public boolean getVideoPrepared() {
        if (mVideoRender == null || mVideoRender.isPrepared == false)
            return false;
        return true;
    }
    public long onGetDuration() {
        //mRenderer.onPlay();
        return mVideoRender.getDurationMedia();
    }
    public void setSeekTo(int seekNum) {
        mVideoRender.onSeekTo(seekNum);
    }
}
